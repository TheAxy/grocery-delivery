...
export function ReduxAppStateProvider({ children }: PropsWithChildren) {
  return <Provider store={reduxStore}>{children}</Provider>
}

export function useReduxSessionModel() {
...
export function useReduxCartModel() {
  const dispatch = useAppDispatch()
...
export function useReduxProductsResource(search = '', category = ''): ResourceResult<Product[]> {
...
export function useReduxOrdersResource(): ResourceResult<Order[]> {
  const token = useAppSelector((state) => state.auth.token)
  const user = useAppSelector((state) => state.auth.user)
...
export function useReduxOrderActions() {
...
export function useReduxAdminProductActions() {
...
export function useReduxSharedStats() {
  const user = useAppSelector((state) => state.auth.user) as UserPublic | null
  const orders = useReduxOrdersResource()
  const products = useReduxProductsResource()
